...
> **Sources:** RE `o12.N(Z)` (handshake), `ri1.d(mac, ctx)` (known device), `DeviceListFragment.e5()` (PIN), `o12.O()` (set time). Super `TripperBleManager.startHandshake()`, `TripperProtocol.buildPinPacket()`, `isDeviceKnown()`.
...
- RE: `ri1.d(mac, ctx)` / SharedPreferences `mytbtlist`
- Super: `TripperKnownDevices` / `knownMacs`
...
### Timing (from `TripperBleManager.startHandshake` / `o12.N`)
