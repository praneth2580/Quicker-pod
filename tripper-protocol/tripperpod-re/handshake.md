...
> **RE source:** `o12.N(Z)` (handshake), `ri1.d(mac, ctx)` (known device check), `DeviceListFragment.e5()` (PIN packet), `o12.O()` (set time). Super equivalents in `smali-reference/super/`.

## Overview

After GATT connection and service discovery, the app runs `startHandshake()` (`o12.N` in RE). The path depends on whether the device MAC is in the **known devices** list (`ri1.d` / SharedPreferences `mytbtlist`).
